...
function(virtualenv_create)
  ...
  cmake_parse_arguments(PYTHON_VENV "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  ...
  if(NOT EXISTS ${PYTHON_VENV_REQUIREMENTS})
    message(
      FATAL_ERROR
        "REQUIREMENTS must exist, invalid path: ${PYTHON_VENV_REQUIREMENTS}")
  endif()
  ...
  add_custom_command(
    ...
    COMMAND ${PYTHON_VENV_INTERPRETER} -m pip install -r
...
function(targets_get_all)
  ...
  get_directory_property(allTargets DIRECTORY ${ARGS_DIRECTORY}
                                              BUILDSYSTEM_TARGETS)
  set(${ARGS_RESULT}
...
function(targets_filter_for_sources)
  ...
  foreach(target IN LISTS ARGS_TARGETS)
    ...
    # Interface targets can propagate sources in cmake 3.19 onward but not
    # compile directly
...
function(targets_get_translation_units)
  ...
  foreach(cppExt IN LISTS CMAKE_CXX_SOURCE_FILE_EXTENSIONS)
    ...
    # Escape any file extensions with special characters (i.e. the '+' in
    # "c++").
  ...
  foreach(file IN LISTS targetTranslationUnits)
    targets_relative_path_of_source(TARGET_NAME ${ARGS_TARGET} RESULT file
                                    SOURCE_FILE ${file})
    if(IS_MULTI_CONFIG)
      set(translationUnitLocation
          "${targetBinaryDir}/CMakeFiles/${ARGS_TARGET}.dir/$<CONFIG>/${file}")
    else()
      set(translationUnitLocation
          "${targetBinaryDir}/CMakeFiles/${ARGS_TARGET}.dir/${file}")
    endif()
...
function(targets_relative_path_of_source)
  ...
  # Generated files should be in the subdirectories of the targets binary
  # directory
...
function(coverage_target_clean_intermediate_file)
  ...
  targets_get_translation_units(TARGET ${COVERAGE_TARGET_NAME} RESULT
                                targetSources)
...
function(enable_code_coverage)
  ...
  if(NOT GENHTML_BIN)
    message(
      SEND_ERROR "Unable to find genhtml, disable code coverage to continue")
  endif()

  find_program(CPPFILT_BIN c++filt)
  if(NOT CPPFILT_BIN)
    message(
      SEND_ERROR "Unable to find c++filt, disable code coverage to continue")
  endif()
  ...
  set(COVERAGE_FASTCOV_TO_SONARQUBE_BIN
      ${COVERAGE_VENV}/bin/fastcov_to_sonarqube)
  set(COVERAGE_LCOV_TO_COBERTURA_BIN ${COVERAGE_VENV}/bin/lcov_cobertura)
  virtualenv_create(
    DESTINATION
    ${COVERAGE_VENV}
    REQUIREMENTS
    ${PROJECT_SOURCE_DIR}/cmake/requirements/coverage_requirements.txt
    WORKING_DIRECTORY
    ${CMAKE_BINARY_DIR}
    OUTPUT
  ...
  foreach(target IN LISTS targetsWithSource)
    ...
    coverage_target_clean_intermediate_file(
      TARGET_NAME ${target} RETURN_DATA_FILES outputGCovDataFile)
  ...
  add_custom_target(
    coverage-clean
    COMMAND ${COVERAGE_FASTCOV_BIN} --gcov ${GCOV_BIN} --exclude
            ${CMAKE_BINARY_DIR} --include ${PROJECT_SOURCE_DIR} --zerocounters
  ...
  add_custom_target(
    coverage-fastcov
    COMMAND ${COVERAGE_FASTCOV_BIN} --gcov ${GCOV_BIN} --include
            ${PROJECT_SOURCE_DIR} -o ${COVERAGE_REPORT_PATH}
  ...
  add_custom_command(
    ...
    COMMAND
      ${COVERAGE_FASTCOV_BIN} --gcov ${GCOV_BIN} --exclude
      ${CMAKE_CURRENT_BINARY_DIR} --include ${PROJECT_SOURCE_DIR} --lcov
      --verbose -o ${LCOV_REPORT_PATH}
  ...
  add_custom_command(
    ...
    COMMAND ${GENHTML_BIN} -o ${LCOV_HTML_PATH} ${LCOV_REPORT_PATH}
            --demangle-cpp
  ...
  add_custom_command(
    ...
    COMMAND ${COVERAGE_LCOV_TO_COBERTURA_BIN} ${LCOV_REPORT_PATH} --base-dir
            ${PROJECT_SOURCE_DIR} --output ${COBERTURA_REPORT_PATH} --demangle
