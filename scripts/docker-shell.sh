...
echo "  CXX=g++-16 CC=gcc-16 ./scripts/cmake.sh --release --implementation=reflection -B build-reflection"
echo ""

DOCKER_INTERACTIVE_FLAGS="-it"
# If not running in a terminal, drop -it
if [ ! -t 0 ]; then
    DOCKER_INTERACTIVE_FLAGS="-i"
fi

exec docker run $DOCKER_INTERACTIVE_FLAGS --rm \
